...
class GoldCalculationEngine {
  ...
  Rational netWeight({required Rational grossWeight, Rational? stoneWeight}) {
    final stone = stoneWeight ?? Rational.zero;
    _requirePositive(grossWeight, 'E001', 'وزن باید بیشتر از صفر باشد.');
    if (stone.isNegative)
      throw const GoldCalcException('E004', 'وزن سنگ نمی‌تواند منفی باشد.');
    if (stone > grossWeight) {
      throw const GoldCalcException('E008', 'وزن سنگ بیشتر از وزن کل است.');
    }
    return grossWeight - stone;
  }
  ...
  CalculationResult calculateSale(SaleInput input) {
    _validateSale(input);
    final net = netWeight(
        grossWeight: input.grossWeight, stoneWeight: input.stoneWeight);
    ...
    if (input.taxRules.discountTaxTreatment ==
        DiscountTaxTreatment.reduceTaxableBase) {
      discount = _discountAmount(
          input.discount, grossBeforeDiscountTax, labor, profit, commission);
      final applicable = _discountAgainstScope(
          input.discount.scope, discount, labor, profit, commission);
      ...
      discount = _discountAmount(
          input.discount, grossAfterTax, labor, profit, commission);
      discount = input.discount.scope == DiscountScope.total
    ...
    final finalTotal =
        _roundMoney(finalTotalRaw, input.roundingUnit, input.roundingMode);
    ...
    return CalculationResult(
      ...
      snapshot: _saleSnapshot(input, net, itemRate, metal, labor, profit,
          commission, taxBase, tax, gross, discount, finalTotal),
    );
  }
  ...
  BuyResult calculateBuy(BuyInput input) {
    ...
    final feeAmount =
        _feeAmount(input.feeMode, input.fee, input.weight, afterDeduction);
  ...
  ExchangeResult calculateExchange(ExchangeInput input) {
    if (input.oldItems.isEmpty || input.newItems.isEmpty) {
      throw const GoldCalcException(
          'E014', 'تعویض باید حداقل یک قطعه قدیمی و یک قطعه جدید داشته باشد.');
    }
  ...
  MeltedResult calculateMelted(MeltedInput input) {
    ...
    final adjusted =
        input.lossEnabled ? pure * (Rational.one - input.lossPercent) : pure;
  ...
  ConversionResult convertPurity(
      {required Rational weight,
      required Rational fromPurity,
      required Rational toPurity}) {
    ...
    return ConversionResult(
        ...
        convertedPricePerGram: Rational.zero);
  }

  ConversionResult convertPrice(
      {required Rational price,
      required PriceBasis from,
      required Rational targetPurity}) {
    _validatePurity(targetPurity);
    final rate =
        RateInput(value: price, basis: from, timestamp: DateTime.now());
  ...
  Rational gram18ToMesghal17({required Rational price18}) =>
      price18 *
      Rational.parse('4.608') *
      (Rational.fromInt(705) / Rational.fromInt(750));

  ReverseResult solveWeightForTarget(
      {required SaleInput baseInput,
      required Rational target,
      Rational? tolerance}) {
    final tol = tolerance ?? Rational.fromInt(1) / Rational.fromInt(1000);
    _requireNonNegative(target, 'target');
    var low = Rational.zero;
    var high = Rational.fromInt(1);
    for (var i = 0;
        i < 160 &&
            calculateSale(_copySale(baseInput, grossWeight: high)).finalTotal <
                target;
        i++) {
      high = high * Rational.fromInt(2);
    }
    if (calculateSale(_copySale(baseInput, grossWeight: high)).finalTotal <
        target) {
      throw const GoldCalcException(
          'E015', 'برای مبلغ هدف، وزن قابل حل در محدوده موتور پیدا نشد.');
    }
    for (var i = 0; i < 220; i++) {
      final mid = (low + high) / Rational.fromInt(2);
      final total =
          calculateSale(_copySale(baseInput, grossWeight: mid)).finalTotal;
    ...
    return ReverseResult(
        ...
        value: mid,
        achievedTotal: total,
        absoluteError: (total - target).abs());
  }

  ReverseResult solveLaborPercentForTarget(
      {required SaleInput baseInput,
      required Rational target,
      Rational? tolerance,
      Rational? lowPercent,
      Rational? highPercent}) {
    final tol = tolerance ?? Rational.fromInt(1) / Rational.fromInt(1000);
    final low = lowPercent ?? Rational.zero;
    final high = highPercent ?? Rational.fromInt(100);
    return _solveMonotonic(
      variable: 'labor_percent',
      target: target,
      low: low,
      high: high,
      valueOf: (x) => calculateSale(_copySale(baseInput,
              labor: LaborInput(mode: LaborMode.percent, rate: x)))
          .finalTotal,
      tolerance: tol,
      rebuild: (x) => calculateSale(_copySale(baseInput,
          labor: LaborInput(mode: LaborMode.percent, rate: x))),
    );
  }

  ReverseResult solveProfitPercentForTarget(
      {required SaleInput baseInput,
      required Rational target,
      Rational? tolerance,
      Rational? lowPercent,
    ...
    return _solveMonotonic(
      ...
      valueOf: (x) => calculateSale(_copySale(baseInput,
              profit: ProfitInput(
                  mode: ProfitMode.percentOfGoldPlusLabor, rate: x)))
          .finalTotal,
      tolerance: tol,
      rebuild: (x) => calculateSale(_copySale(baseInput,
          profit:
              ProfitInput(mode: ProfitMode.percentOfGoldPlusLabor, rate: x))),
    );
  }

  ReverseResult solveDiscountForTarget(
      {required SaleInput baseInput,
      required Rational target,
      Rational? tolerance}) {
    final noDiscount = calculateSale(
            _copySale(baseInput, discount: DiscountInput(value: Rational.zero)))
        .grossTotal;
    final required = (noDiscount - target).max(Rational.zero);
    if (required > noDiscount)
      throw const GoldCalcException(
          'E016', 'تخفیف هدف بیشتر از مبلغ معامله است.');
    final achieved = calculateSale(_copySale(baseInput,
            discount: DiscountInput(mode: DiscountMode.fixed, value: required)))
        .finalTotal;
    return ReverseResult(
        variable: 'discount',
        value: required,
        achievedTotal: achieved,
        absoluteError: (achieved - target).abs());
  }

  Rational roundMoney(Rational amount, int unit, RoundingMode mode) =>
      _roundMoney(amount, unit, mode);
  ...
  Rational _rateAs24k({required RateInput rate}) {
    switch (rate.basis) {
      ...
      case PriceBasis.perGivenPurity:
        throw const GoldCalcException('E017',
            'برای نرخ perGivenPurity باید عیار مبنا نیز در ورودی مشخص باشد.');
      case PriceBasis.perMesghal17k:
        return mesghal17To18(mesghalPrice: rate.value) *
            Rational.fromInt(1000) /
            Rational.fromInt(750);
    }
  }
  ...
  Rational _discountAmount(DiscountInput discount, Rational gross,
      Rational labor, Rational profit, Rational commission) {
    ...
    return discount.mode == DiscountMode.percent
        ? base * discount.value
        : discount.value;
  }

  Rational _discountAgainstScope(DiscountScope scope, Rational discount,
      Rational labor, Rational profit, Rational commission) {
  ...
  Rational _feeAmount(
      FeeMode mode, Rational fee, Rational weight, Rational base) {
  ...
  Rational _clampDiscount(Rational discount, Rational gross) =>
      discount.min(gross).max(Rational.zero);
  ...
  Rational _roundMoney(Rational amount, int unit, RoundingMode mode) {
    ...
    return Rational(whole * BigInt.from(unit));
  }

  ReverseResult _solveMonotonic(
      {required String variable,
      required Rational target,
      required Rational low,
      required Rational high,
      required Rational Function(Rational) valueOf,
      required CalculationResult Function(Rational) rebuild,
      required Rational tolerance}) {
    _requireNonNegative(target, 'target');
    final lowValue = valueOf(low);
    final highValue = valueOf(high);
    if (target < lowValue || target > highValue) {
      throw GoldCalcException(
          'E018', 'مبلغ هدف خارج از محدوده قابل دستیابی برای $variable است.');
    }
    var a = low;
    var b = high;
    for (var i = 0; i < 220; i++) {
      ...
      if (diff.abs() <= tolerance) {
        ...
        return ReverseResult(
            variable: variable,
            value: mid,
            achievedTotal: result.finalTotal,
            absoluteError: (result.finalTotal - target).abs());
      }
      if (value < target) {
        a = mid;
      } else {
  ...
  SaleInput _copySale(SaleInput source,
          {Rational? grossWeight,
          LaborInput? labor,
          ProfitInput? profit,
  ...
  void _validateSale(SaleInput input) {
    ...
    if (input.rate.value.isNegative)
      throw const GoldCalcException('E003', 'نرخ نمی‌تواند منفی باشد.');
    if (input.rate.value.isZero)
      throw const GoldCalcException('E003', 'نرخ باید بیشتر از صفر باشد.');
  }
  ...
  void _validateMelted(MeltedInput input) {
    _requirePositive(input.weight, 'E001', 'وزن باید بیشتر از صفر باشد.');
    _validatePurity(input.purity);
    if (input.rate.value.isNegative || input.rate.value.isZero)
      throw const GoldCalcException('E003', 'نرخ باید بیشتر از صفر باشد.');
    _requireNonNegative(input.lossPercent, 'loss_percent');
    if (input.lossPercent > Rational.one)
      throw const GoldCalcException(
          'E019', 'کسر ذوب نمی‌تواند بیشتر از 100 درصد باشد.');
  }

  void _validatePurity(Rational purity) {
    if (purity <= Rational.zero || purity > Rational.fromInt(1000)) {
      throw const GoldCalcException(
          'E002', 'عیار باید بیشتر از صفر و حداکثر 1000 باشد.');
    }
  }
  ...
  void _requireNonNegative(Rational value, String name) {
    if (value.isNegative)
      throw GoldCalcException('E004', '$name نمی‌تواند منفی باشد.');
  }
  ...
  Map<String, dynamic> _saleSnapshot(
          SaleInput input,
          Rational net,
          Rational itemRate,
          Rational metal,
