                  ...
                  child: Padding(
                    ...
                    child: Column(
                        ...
                        children: [
                          ...
                          Text(signUp ? 'ساخت حساب' : 'ورود به حساب',
                        ...
                        child: Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              const SizedBox(height: 12),
                              const Text('اشتراک فعال نیست',
                                  style: TextStyle(
                                      fontSize: 24,
                                      fontWeight: FontWeight.w900)),
                              const SizedBox(height: 10),
                              const Text(
                                  'برای استفاده از ماشین‌حساب حرفه‌ای باید اشتراک فعال داشته باشید.',
                                  textAlign: TextAlign.center),
                              const SizedBox(height: 20),
                              FilledButton.icon(
                                  onPressed: widget.preview ? null : refresh,
                                  icon: const Icon(Icons.refresh),
                                  label: const Text('بررسی دوباره اشتراک')),
                              const SizedBox(height: 8),
                              if (!widget.preview)
                                TextButton(
                                    onPressed: () =>
...
class CalculatorHome extends StatefulWidget {
  ...
  const CalculatorHome(
      {super.key, required this.plan, required this.onDeactivate});
...
class _CalculatorHomeState extends State<CalculatorHome> {
  ...
  SaleInput saleInput() => SaleInput(
        ...
        labor: LaborInput(
            ...
            rate: percent(r(labor.text).toDecimalString())),
        profit: ProfitInput(
            mode: ProfitMode.percentOfGoldPlusLabor,
            rate: percent(r(profit.text).toDecimalString())),
        taxRules: EngineRules.mvp1405().taxRuleSet,
        discount: DiscountInput(
            value: r(discount.text),
  ...
  void _calculateSale() {
    ...
    _setResult(
      ...
      detail:
          '${r(weight.text).toDecimalString()} گرم • عیار ${r(purity.text).toDecimalString()}',
    );
  }
  ...
  void _calculateBuy() {
    ...
    _setResult(
      ...
      detail:
          '${r(buyWeight.text).toDecimalString()} گرم • ضریب خرید ${r(buyFactor.text).toDecimalString()}',
    );
  }
  ...
  void _calculateExchange() {
    ...
    final x = engine.calculateExchange(
        ExchangeInput(oldItems: [oldItem], newItems: [newItem]));
    final direction = x.cashDifference > Rational.zero
        ? 'مشتری پرداخت می‌کند'
        : x.cashDifference < Rational.zero
            ? 'مشتری دریافت می‌کند'
            : 'تسویه صفر';
    _setResult(
      ...
      detail:
          '${r(oldWeight.text).toDecimalString()} گرم قدیم ← ${r(newWeight.text).toDecimalString()} گرم جدید',
    );
  }
  ...
  void _calculateMelted() {
    ...
    final x = engine.calculateMelted(
      MeltedInput(
        ...
        rate: RateInput(
            value: r(rate24.text), basis: PriceBasis.per24k, timestamp: now),
        feeMode: FeeMode.fixed,
        fee: r(meltFee.text),
    ...
    _setResult(
      ...
      detail:
          '${r(meltedWeight.text).toDecimalString()} گرم • عیار ${r(meltedPurity.text).toDecimalString()}',
    );
  }
  ...
  void reverseByBudget() {
    try {
      if (mode != CalcMode.sale) {
        setState(
            () => status = 'محاسبه معکوس بودجه فعلاً برای حالت فروش فعال است.');
        return;
      }
      final x = engine.solveWeightForTarget(
          baseInput: saleInput(), target: r(budget.text));
      setState(() => status =
          'با بودجه ${_group(r(budget.text).roundHalfUp().toString())} تومان ≈ ${x.value.toDecimalString(scale: 3)} گرم');
    } catch (e) {
      setState(() => status = e.toString());
    }
  }
  ...
  void convertPurity() {
    try {
      ...
      _showMessage(
          'خروجی تبدیل: ${grams(x.convertedWeight)} • طلای خالص: ${grams(x.pureWeight)}');
    } catch (e) {
  ...
  void saveSettings() {
    setState(() {
      currentStore =
          currentStore.trim().isEmpty ? 'فروشگاه طلا' : currentStore.trim();
        ...
        actions: [
          ...
          PopupMenuButton<String>(
            ...
            itemBuilder: (context) => const [
              ...
              PopupMenuItem(
                  value: 'deactivate', child: Text('قفل نمایشی اشتراک')),
            ],
            child: const Padding(
                padding: EdgeInsets.all(12),
                child: CircleAvatar(radius: 17, child: Text('پ'))),
          ),
        ],
      );
        ...
        child: Padding(
          ...
          child: Wrap(
            ...
            children: [
              ...
              SizedBox(
                  width: 190,
                  child: _field('قیمت ۱۸ عیار', rate18, suffix: 'تومان')),
              SizedBox(
                  width: 190,
                  child: _field('قیمت ۲۴ عیار', rate24, suffix: 'تومان')),
              SizedBox(
                  width: 190, child: _field('مظنه', mesghal, suffix: 'تومان')),
              Chip(
                  label: Text(widget.plan == 'آزمایشی'
                      ? 'اشتراک آزمایشی'
                      : 'اشتراک ${widget.plan}')),
            ],
          ),
  ...
  Widget _modeTabs() => SingleChildScrollView(
        ...
        child: SegmentedButton<CalcMode>(
          segments: const [
            ...
            ButtonSegment(
                value: CalcMode.buy,
                label: Text('خرید از مشتری'),
                icon: Icon(Icons.handshake_outlined)),
            ButtonSegment(
                value: CalcMode.exchange,
                label: Text('تعویض'),
                icon: Icon(Icons.sync_alt)),
            ButtonSegment(
                value: CalcMode.melted,
                label: Text('آبشده'),
    ...
    switch (mode) {
      case CalcMode.sale:
        return _card(
          ...
          children: [
            _twoFields(
                _field('وزن', weight, suffix: 'گرم'), _field('عیار', purity)),
            const SizedBox(height: 10),
            _twoFields(_field('اجرت', labor, suffix: '%'),
                _field('سود', profit, suffix: '%')),
            const SizedBox(height: 10),
      ...
      case CalcMode.buy:
        return _card(
          ...
          children: [
            _twoFields(_field('وزن', buyWeight, suffix: 'گرم'),
                _field('عیار', buyPurity)),
            const SizedBox(height: 10),
            _twoFields(_field('ضریب خرید', buyFactor),
                _field('کسر ثابت', buyDeduction, suffix: 'تومان')),
            const SizedBox(height: 10),
        ...
        return _card(
          ...
          children: [
            const Align(
                ...
                child: Text('طلای قدیمی مشتری',
                    style: TextStyle(fontWeight: FontWeight.w700))),
            const SizedBox(height: 8),
            _twoFields(_field('وزن قدیمی', oldWeight, suffix: 'گرم'),
                _field('عیار قدیمی', oldPurity)),
            const SizedBox(height: 12),
            const Align(
                alignment: Alignment.centerRight,
                child: Text('طلای جدید',
                    style: TextStyle(fontWeight: FontWeight.w700))),
            const SizedBox(height: 8),
            _twoFields(_field('وزن جدید', newWeight, suffix: 'گرم'),
                _field('عیار جدید', newPurity)),
            const SizedBox(height: 12),
            _actionRow(),
          ],
        );
      case CalcMode.melted:
        return _card(
          ...
          children: [
            _twoFields(_field('وزن', meltedWeight, suffix: 'گرم'),
                _field('عیار', meltedPurity)),
            const SizedBox(height: 10),
            _twoFields(_field('کسری ذوب', meltLoss, suffix: '%'),
                _field('هزینه', meltFee, suffix: 'تومان')),
            const SizedBox(height: 12),
            const Text(
                'در این حالت قیمت ۲۴ عیارِ واردشده در نوار بالا استفاده می‌شود.',
  ...
  Widget _twoFields(Widget a, Widget b) => Row(children: [
        Expanded(child: a),
      ...
      Card(
        child: Padding(
          ...
          child:
            ...
            Row(children: [
              Icon(icon, color: const Color(0xFFC58B2A)),
              const SizedBox(width: 8),
              Text(title,
                  style: const TextStyle(
          ...
          child:
            ...
            if (mode == CalcMode.sale) ...[
              Row(children: [
                Expanded(
                    child: OutlinedButton.icon(
                        onPressed: reverseByBudget,
                        icon: const Icon(Icons.swap_horiz),
                        label: const Text('محاسبه معکوس'))),
                const SizedBox(width: 10),
                Expanded(
                    child: OutlinedButton.icon(
                        onPressed: () => _showMessage(
                            'اشتراک‌گذاری در نسخه انتشار متصل می‌شود.'),
  ...
  Widget _quickActions() => Wrap(spacing: 10, runSpacing: 10, children: [
        ...
        _quick(
            ...
            'مظنه ↔ گرم',
            () => _showMessage(
                'تبدیل مظنه با نرخ دستی در نسخه بعدی تکمیل می‌شود.')),
        _quick(Icons.scale_outlined, 'محاسبه وزن', reverseByBudget),
        _quick(
            Icons.percent,
            'محاسبه اجرت',
            () => _showMessage(
                'اجرت را در حالت فروش وارد کنید؛ محاسبه معکوس هم از همان‌جا در دسترس است.')),
        _quick(Icons.history, 'تاریخچه',
            () => setState(() => tab = AppTab.history)),
        _quick(Icons.tune, 'تنظیمات مغازه',
            () => setState(() => tab = AppTab.settings)),
      ]);
            ...
            Card(
              child: Padding(
                ...
                child: Column(children: [
                  ...
                  _twoFields(
                      _field('اجرت پیش‌فرض',
                          TextEditingController(text: defaultLabor),
                          suffix: '%'),
                      _field('سود پیش‌فرض',
                          TextEditingController(text: defaultProfit),
                          suffix: '%')),
                  const SizedBox(height: 10),
                  SwitchListTile.adaptive(
                      value: haptic,
                      onChanged: (v) => setState(() => haptic = v),
                      title: const Text('بازخورد لمسی')),
                  SwitchListTile.adaptive(
                      value: showWarnings,
                      onChanged: (v) => setState(() => showWarnings = v),
                      title: const Text('هشدار ورودی‌های غیرعادی')),
                  const SizedBox(height: 8),
                  SizedBox(
              ...
              child: Column(children: [
                const ListTile(
                    title: Text('وضعیت اشتراک',
                        style: TextStyle(fontWeight: FontWeight.w700)),
                    trailing: Chip(label: Text('فعال'))),
                ListTile(
                    leading: const Icon(Icons.workspace_premium_outlined),
                    title: Text('پلن فعلی: ${widget.plan}'),
                    subtitle: const Text(
                        'در نسخه واقعی به Supabase/پرداخت متصل خواهد شد.')),
                ListTile(
                    leading: const Icon(Icons.wifi_off_outlined),
                    title: const Text('محاسبات آفلاین'),
                    subtitle: Text(showWarnings
                        ? 'فعال • بدون وابستگی به API قیمت'
                        : 'فعال')),
              ]),
            ),
          ],
...
extension on SaleInput {
  SaleInput copyWith(
          {Rational? grossWeight, Rational? purity, DiscountInput? discount}) =>
      SaleInput(
